...
public:
...
#if _MSC_VER >= 1300
	virtual LPCWSTR  GetText() const      { ASSERT( FALSE); return NULL; } // must override
#else
    ...
    virtual void SetData(LONGLONG lParam)
    ...
    {
        if(plf != NULL && m_pTreeColumn != NULL)
        {
            ...
            m_pTreeColumn->SetDefTreeIndent( tm.tmHeight+tm.tmExternalLeading - 2);    // so can draw square box
    ...
    virtual LONGLONG   GetData() const
        {
