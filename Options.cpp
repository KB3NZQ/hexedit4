...
// First the user can set various values for the active file in the
...
void COptSheet::page_init()
{
	...
	AddPageToTree(pCatPrn, &printDecorationsPage_, IMG_PRN_DECO, IMG_PRN_DECO);
...
void CColourSchemes::set_scheme()
{
	...
	ASSERT(plist->GetCount() == INDEX_LAST);  // make sure when we add/remove colours that the list box strings are OK
