...
BOOL CResizeCtrl::Create( HWND hWndParent, BOOL enable, int maxPart, BOOL isPropertyPage )
{
	...
	m_minTracking.cx  =
		...
		m_maxTracking.cx  =
		m_maxTracking.cy  = -1;
    m_borderSize.cx = m_borderSize.cy = -1;
...
void CResizeCtrl::Resize(int cx, int cy)
{
    ...
    if (m_borderSize.cx == -1)
    {
	    RECT rect, cli;
	    ::GetWindowRect( m_hWndParent, &rect );
	    ::GetClientRect( m_hWndParent, &cli );
        m_borderSize.cx = (rect.right - rect.left) - (cli.right - cli.left);
        m_borderSize.cy = (rect.bottom - rect.top) - (cli.bottom - cli.top);
    }
	// Don't resize if sizes smaller than allowed
	if (cx < m_minTracking.cx - m_borderSize.cx)
...
BOOL  CResizeCtrl::SetInitialSize( const CSize & size )
{
	m_size = size;
	return TRUE;
}
