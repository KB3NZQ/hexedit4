...
typedef struct _GV_ITEM {
    ...
    LONGLONG   lParam;      // 32-bit value to associate with item
...
typedef struct _GV_ITEMW {
    ...
    LOGFONT  lfFont;      // Cell font
    UINT     nMargin;     // Internal cell margin
	CStringW strText;     // This allows to display wide strings even in ANSI builds
} GV_ITEMW;
#endif
...
#if _MSC_VER >= 1300
    virtual void SetText(LPCWSTR /* szText */)              { ASSERT( FALSE);    }
#endif
...
#if _MSC_VER >= 1300
    virtual LPCWSTR  GetText() const                        { ASSERT( FALSE); return NULL;  }
#else
    virtual LPCTSTR  GetText() const                        { ASSERT( FALSE); return NULL;  }
#endif
    virtual LPCTSTR  GetTipText() const                     { return CString(GetText());             } // may override TitleTip return
    virtual int      GetImage() const                       { ASSERT( FALSE); return -1;    }
    virtual LONGLONG   GetData() const                        { ASSERT( FALSE); return 0;     }
