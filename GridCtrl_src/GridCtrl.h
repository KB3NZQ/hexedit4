...
#include <set>  // For m_SelMap, m_PrevSelMap
#include <vector>  // for m_col_size
...
public:
    ...
    int GetUserColumnWidth(int nCol) const
    {
        ASSERT(nCol >= 0 && nCol < m_nCols);
        if (m_arColWidths[nCol] == 0)
            return 0;
        else
            return m_col_size[nCol];
    }
    void SetUserColumnWidth(int col, int width)
    {
        ASSERT(col >= 0 && col < m_nCols);
        m_col_size[col] = width;
    }
    ...
    inline CSize GetCellTextExtent(int nRow, int nCol)  { return GetTextExtent(nRow, nCol, (CString)GetItemText(nRow,nCol)); }
...
#else
    int GetSelectedCount() const                  { return (int)m_SelMap.size(); }
#endif
    ...
    void SetAutoFit(BOOL bAutoFit = TRUE)         { m_bAutoFit = bAutoFit;            }
...
#if _MSC_VER >= 1300
    BOOL   SetItem(const GV_ITEMW* pItem);
    BOOL   SetItemText(int nRow, int nCol, LPCWSTR strw);
#endif
    // The following was virtual. If you want to override, use 
    //  CGridCellBase-derived class's GetText() to accomplish same thing
#if _MSC_VER >= 1300
    CStringW GetItemText(int nRow, int nCol) const;
#else
...
public:
    ...
    void ExpandColsNice(BOOL bExpandFixed = TRUE, UINT nAutoSizeStyle = GVS_DEFAULT);
...
protected:
    ...
    BOOL        m_bAutoFit;                                       // fit columns when grid resized using ExpandColsNice
    ...
    std::vector<int> m_col_size;      // Size user resized the column to (-1 if never resized)
...
#else
    std::set<DWORD> m_SelMap, m_PrevSelMap;  // Stores all selected cells (loword=row, hiword=col)
#endif
