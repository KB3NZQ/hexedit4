							 ...
							 default_oem_scheme_(OEM_NAME), default_ebcdic_scheme_(EBCDIC_NAME)
{
	...
	algorithm_ = 0;   // Default to first encryption algorithm
...
{
	if (AfxMessageBox("This restores all tool windows so they are\n"
					  "visible, and undocked.  They include:\n"
					  "* Calculator\n"
					  "* Properties\n"
					  "* Bookmarks\n"
					  "* Find dialog\n"
					  "* HexEdit Explorer\n"
					  "\nYou may need to restore them if one"
					  " or more tool windows cannot be found.\n"
...
void CHexEditApp::PreLoadState()
{
	...
	GetMouseManager()->SetCommandForDblClk(IDR_CONTEXT_RULER, ID_ADDR_TOGGLE);       // default to toggle hex/dec addresses in address area and ruler
...
void CHexEditApp::OnEncryptAlg() 
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if (algorithm_ >= 0)
			crypto_.SetPassword(algorithm_, NULL);

		// Get new algorithm and create key based on current password
		algorithm_ = dlg.m_alg;
		ASSERT(algorithm_ >= 0 && algorithm_ < (int)crypto_.GetNum());

		if (!password_.IsEmpty())
		{
			// Set key based on password
			crypto_.SetPassword(algorithm_, password_);
		}
		else
		{
			// Clear password
		...
		SaveToMacro(km_encrypt_alg, crypto_.GetName(algorithm_));
	}
}
...
void CHexEditApp::OnEncryptPassword() 
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		ASSERT(algorithm_ < (int)crypto_.GetNum());
		crypto_.SetPassword(algorithm_, password_);
...
void CHexEditApp::LoadOptions()
{
	...
	alt_data_bg_cols_ = GetProfileInt("DataFormat", "AltDataBgCols", 1) != 0 ? true : false;  // xxx check box in opt dlg
